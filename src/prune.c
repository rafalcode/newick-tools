...
static void prune_taxa(rtree_t **root, rtree_t **prune_tips_list, unsigned int prune_tips_count)
{
    unsigned int i;
    ...
    for (i = 0; i < prune_tips_count; ++i) {
        ...
        if (!grandparent)
            fprintf(stderr, "Warning: All taxa from one root subtree deleted. Root changed.\n");

        rtree_t * temp = (parent->left == prune_tips_list[i]) ?  parent->right : parent->left;
...
static utree_t * utree_prune_taxa(utree_t ** prune_tips_list, unsigned int prune_tips_count)
{
    unsigned int i;
...
void cmd_prune_tips()
{
    ...
    if (!rtree) {
        ...
        if (!utree)
            fatal("Tree is neither unrooted nor rooted. Kindly fix your tree.");
        ...
        if (opt_prune_random) {
            ...
            prune_tips_list = utree_tipstring_nodes(utree, tip_count, opt_prune_tips, &prune_tips_count);
        ...
        if (opt_prune_random) {
            ...
            prune_tips_list = rtree_tipstring_nodes(rtree, opt_prune_tips, &prune_tips_count);
...
void cmd_induce_tree()
{
    ...
    complement_tiplist = rtree_tipstring_nodes(rtree, opt_induce_subtree, &complement_tips_count);

    prune_tiplist = rtree_tiplist_complement(rtree, complement_tiplist, complement_tips_count);
